...
import { createPromo, setPromoActive } from "@/lib/vendor/promo-actions";
...
export function PromosView({
  ...
  async function handleCreate(event: React.FormEvent) {
    ...
    const result = await createPromo(shopId, {
  ...
  async function handleToggle(promo: Promo) {
    const result = await setPromoActive(promo.id, !promo.is_active);
    if (result.error) {
      toast.error(result.error);
      return;
    }
  ...
  return (
    <div className="space-y-4">
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogTrigger render={<Button>New promo</Button>} />
        <DialogContent>
          <DialogHeader>
